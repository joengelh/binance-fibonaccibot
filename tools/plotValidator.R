...
sqlQuery <- dbSendQuery(con, "SELECT * FROM backtesting2 
                     WHERE resultpercent is not null andpricechangepercent >= 0")
validated <- dbFetch(sqlQuery)
dbClearResult(sqlQuery)

for (i in 1:nrow(validated)){
  # get plot data
  sqlString <-  paste("select * from backtesting2 
  ...
  # get stopid time
  sqlString <-  paste("select time from backtesting2 
